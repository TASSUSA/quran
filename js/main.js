(function($){
	...
	$.fn.Quran = function(){
		var $quran = this,
			$quranText = $('#quranText', $quran),//, $transText = $('#transText');
			$sura_select = $('.sura-select', $quran),
			$page_number = $('.page-number', $quran),
			$page_controls = $('.page-controls', $quran),

			$page_left = $page_controls.find('.page-left'),
			$page_right = $page_controls.find('.page-right'),

			$sura_left = $('.sura-left', $quran),
			$sura_right = $('.sura-right', $quran),

			$lang_left = $('.lang-left', $quran),
			$lang_right = $('.lang-right', $quran);

		var last_chapter = 113;
		var chapter = 0,//notice: 0 based, but display is 1 based.
			verse = undefined; //Used when url hash has verse value
		var page = 0, per_page = 10, pages = Math.ceil(verseCounts[chapter] / per_page);

		var lang_left = 'ar_uthmani',
			lang_right = 'en_ahamed';
